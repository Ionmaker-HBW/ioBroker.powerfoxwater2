...
class Powerfox2 extends utils.Adapter {
  ...
  async onReady() {
    ...
    if (!this.config.email || !this.config.password) {
      this.log.error(
        "Error with login datas: Please enter login datas in instance settings.",
      );
      return;
    }
    if (!(this.config.devices && this.config.devices.length)) {
      this.log.error(
        "Error on powerfox devices: Please check powerfox device in instance settings.",
      );
      return;
    }
    // --- ENDE ÄNDERUNG ---
  ...
  getCurrentInterval() {
    ...
    this.log.debug(
      `getCurrentInterval: hour=${hour} nightStart=${nightStart} nightEnd=${
        nightEnd
      } isNight=${isNight}`,
    );
    ...
    for (let i = 0; i < this.config.devices.length; i++) {
      ...
      if (device.active) {
        ...
        try {
          ...
          this.log.debug(
            `API request: GET ${this.apiClient.defaults.baseURL}${curDataUrl}`,
          );
          ...
          const requestDuration = ((Date.now() - requestStart) / 1000).toFixed(
            3,
          );
          this.log.debug(
            `API response: ${result.status} in ${requestDuration}s`,
          );
          // --- ENDE ÄNDERUNG ---
...
*/
            ...
            // NEUCODE:
	    if (data.CubicMeterCold === undefined) {
    		this.log.error(
        	`Incomplete water data received for device: ${device.name}`,
    		);
    		continue;
	    }
            // --- ENDE ÄNDERUNG ---
		...
		this.log.debug(
 		   `coldWater: ${data.CubicMeterCold} m³ | warmWater: ${data.CubicMeterWarm} m³`,
		);
            // ALTCODE (number):
            ...
            if (!existingState) {
              ...
              await this.fsetObjectNotExistsAsync(
                `${path}.deviceType`,
                "state",
                "deviceType",
                "string",
...
await this.fsetObjectNotExistsAsync(
  ...
  async fsetObjectNotExistsAsync(
    varname,
    vartype,
    varcname,
    varctype,
...
function createVarName(text) {
  return text
    .toLowerCase()
    .replace(/\s/g, "_")
    .replace(/[^\x20\x2D0-9A-Z\x5Fa-z\xC0-\xD6\xF8-\xFF]/g, "");
}
